...
from PySide6.QtCore import Signal, Qt
from PySide6.QtGui import QImage, QPixmap
from PySide6.QtWidgets import QLabel, QDialog, QVBoxLayout, QWidget, QGraphicsView, QGraphicsScene, \
    QHBoxLayout, QGroupBox, QGridLayout, QPushButton, QSpacerItem, QSizePolicy
...
class ResultsViewWidget(QWidget):
    ...
    def init_ui(self):
        # Initialize annotated images groupbox and skeleton plot groupbox
        self.annotated_images_groupbox = QGroupBox("Annotated Images")
        self.skeleton_plot_groupbox = QGroupBox("3D Reconstruction")

        self.annotated_images_layout = QGridLayout()
        self.annotated_images_groupbox.setLayout(self.annotated_images_layout)
        self.layout.addWidget(self.annotated_images_groupbox)

        self.skeleton_view = SkeletonViewWidget()
        self.skeleton_view.setMinimumSize(600, 400)
        skeleton_plot_layout = QVBoxLayout()
        skeleton_plot_layout.addWidget(self.skeleton_view)
        self.skeleton_plot_groupbox.setLayout(skeleton_plot_layout)
        self.layout.addWidget(self.skeleton_plot_groupbox)

        self.add_return_to_snapshot_tab_button()
    ...
    def update_annotated_images(self, snapshot_data_2d):
        ...
        for i in reversed(range(self.annotated_images_layout.count())):
            widget = self.annotated_images_layout.itemAt(i).widget()
        ...
        for name, image in snapshot_data_2d.annotated_images.items():
            qt_image, original_pixmap, scaled_pixmap = self.convert_image_for_display(image)
            image_label = ClickableImageLabel(scaled_pixmap, original_pixmap)
            self.annotated_images_layout.addWidget(image_label, row, col)
            col += 1
            if col >= 2:  # Change the number of columns if needed
                row += 1
                col = 0
    ...
    def add_return_to_snapshot_tab_button(self):
        button = QPushButton("Take Another Snapshot")
        button.clicked.connect(self.emit_return_to_snapshot_tab_signal)
